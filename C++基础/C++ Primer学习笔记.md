...
不同变量的实际大小在不同机器上有所差异，**精度决定了我们表达数据的范围**。主要分为：算术类型和空类型（void）
...
当在程序的某处我们使用了一种类型而其实对象应该取另一种类型时，程序便会自动进行数据类型的转换。

### 2.1.3 字面值常量

一个形如42的值便被称作字面值常量，字面值常量的形式和值决定了它的数据类型。
...
变量是具有类型、名称、可操作的**存储空间**（C++ pritmer P38)。每一个变量都有数据类型，数据类型决定了变量所占据内存空间的大小和布局方式、以及值的范围。`C++中，变量和对象一般可以互换使用`

### 2.2.1 变量的定义

```
类型说明符 变量列表（可以赋值）；
int sum， value = 0, sold = 0;
```

**初始化**

初始化不是赋值，初始化含义是创建变量时赋予一个初始值，二赋值的含义是把对象的当前值擦除，而以一个新值来代替。

所以我们的变量可以先定义（此时变量会被默认初始化），再赋值。**即人工初始化是非必要的。**
...
其中，{}来初始化变量得到全面应用，被称为列表初始化，也可以用于赋值操作。

**默认初始化**

如果定义变量没有指定初值，则变量会被默认初始化，默认值由变量类型决定，同时定义变量的位置也会对此有影响。
...
为了把一个程序拆分成多个文程序文件，C++支持分离式编译机制，每个文件可以被独立编译。但程序文件之间可能存在共用的变量，声明就使得该变量为当前程序文件所知道，一个文件如果想使用别处定义的变量，则必须包含对那个变量的声明。**声明像是做虚拟的外部映射标记，而定义负责创建与变量关联的实体。**

**与定义异同**

相同点：都需要规定变量的类型和名字

不同点：定义需要申请存储空间，比如为变量赋一个初始值

**如何去声明？**

如果声明一个变量而非定义它，在变量前加`extern`，且不要显式地初始化变量，比如：
...
所谓标识符就是给变量起的名字，由字母、数字和下划线组成，必须以字母或者下划线开头，对大小写敏感，且不能与标准库冲突。
...
- 变量名一般小写字母
- 用户自定义的类名一般以大写字母开头
- 如果由多个单词构成，单词间需要有明显的区分。如student_loan、studentLoan
...
无论程序的什么位置，使用到的每一个名字都会指向一个特定的实体：变量、函数、类型等。然而，同一个名字如果出现在程序的不同位置，也可能指向不同的实体。

**作用域**

C++做作用域用{}表示，其中的名字都有其特定的含义。

同一个名字在不同作用域中可能指向不同的实体。名字的有效区域始于名字的声明语句，以声明语句所在作用域的末端为结束。比如：
...
这段程序定义了三个名字：main，sum和val；同时使用了std命名空间，该空间提供两个名字cout和cin供程序使用。

名字main定义于所有花括号之外，拥有全局作用。一旦声明之后，全局作用域的名字可以在整个程序中使用；

名字sum定义于main函数所限定的作用域内，从声明sum开始到main函数截止都可以访问它，但出了main函数就无法访问了；

名字val定义于for语句内，在for语句之内可以访问val，但在main函数其他部分就不能访问了。

```
在变量第一次被使用的地方去定义它是一个好的选择，因为我们更容易找到他的作用范围。
```

**嵌套作用域**

- 作用域能彼此包含，被包含的作用域叫内层作用域，包含着别的作用域叫外层作用域。

- 作用域中一旦声明了某个名字，它所嵌套的所有作用域都能访问该名字。同时，也允许内层作用域重新定义外层作用域已有的名字。
...
符合类型是指基于其他类型定义的类型。主要有：引用和指针。说白了就是参考已有变量类型去定义一个新的变量。

**之前的变量定义：类型说明符+变量列表；**

**现在的变量定义：基本数据类型+声明符列表;**

每一个声明符命名一个变量并指定该变量为基本数据类型有关的数据类型。**所以引用和指针就是声明符(类型修饰符）了。**

### 2.3.3 引用

引用即别名。即为对象起了另一个名字，通过将声明符写成&d的形式来定义引用类型，其中d是声明的变量名。比如：

```
int val = 10;
int &refVal = ival; //refVal指向val，即是val的另一个名字
...
指针是一种指向另外一种类型的复合类型，与应用区别有：

- 指针本身是一个对象，允许对指针赋值和拷贝，而且在其生命周期内可以指向几个不同的对象。
- 指针无需在定义时赋初值，也会有默认初始化。

定义指针方法是将声明符写成`*d`的形式，其中d是变量名。

**获取对象的地址**

指针存放着某个对象的地址，要想获取该地址，需要使用取地址符&。

```
int a = 2;
int *p = &p;  // p存放着变量a的地址，或者说p指向变量a的指针。
```
...
- 引用本身不是一个对象，一旦定义就无法再绑定到其他对象上。
- 指针和其他变量一样吗，可以重新指向一个新的对象。

**指针的赋值**

要搞明白赋值语句到底改变了指针的值还是改变了指针所指的对象的值，记住赋值永远改变的是等号左侧的对象。

```
int *pi = 0;

pi = &ival; // pi的值被改变了，现在pi指向了ival
*pi = 0; // ival的值被改变了，指针pi的值并没有改变
```
...
变量的定义包括一个基本数据类型和一组声明符，在同一条定义语句中，虽然基本数据类型只有一个，但声明符形式却可以不同，即一条定义语句可以定义出不同类型的变量，例如：
...
通过*的个数区分指针的级别，`**`表示指向指针的指针吗，`***`表示指向指针的指针的指针



## 2.4 const限定符

我们希望定义一个值不能被改变的变量，比如用它保存缓冲区的大小。为啥要用变量呢？当我们觉得缓冲区大小不太合适时候，我们还可以很容易去修改它。另一方面，也防止程序一不小心改变这个值。可以用关键字`const`对变量的类型加以限定：

```
const int bufsize = 512;
```

这样就把bufSize定义为一个常量，任何对它赋值操作都会引发错误：

````
bufSize = 512 // 错误
````

因为const对象一旦创建就不可以再改变，所有const对象必须初始化，初始化方式可以是任意复杂的表达式。

### 2.4.1 const的引用

可以把引用绑定到const对象上，称之为对常量的引用。与普通引用不同的是，对常量的引用不能被 用作修改它所绑定对象。比如：
...
因为不允许直接为ci赋值，当然也就不能通过引用改变它。

**初始化和对const的引用**

一般来说，引用类型必须与其引用对象的类型一致，但也有例外：

- 在初始化常量引用时允许任意表达式作为初始值，只要该表达式的结果能转换成引用类型即可。
- 允许一个常量引用绑定非常量的对象或字面值，甚至一个表达式。

可见常量引用的初始化（即引用对象的类型）还是很随意的，但引用类型还是很严格的！！！

### 2.4.2 指针与const

与引用一样，也可以让指针指向常量或者非常量，指向常量的指针不能用于改变其所指向对象的值。

**const指针**

指针是对象而不是引用，因而指针本身可以定义为常量。把*放在const关键字之前说明指针是一个常量，表示不变的是指针本身的值而非指向的那个值。

常量指针必须初始化，而且一旦初始化完成，他的值就不能再改变了。比如：

```
int num = 0;
int *const cur = &num; // cur将一直指向num
...
用名词`顶层const`表示指针本身是一个常量，`底层const`表示指针所指的对象是一个常量。
...
auto让编译器通过初始值推算变量的类型，故auto定义的变量必须有初始值。
...
希望从表达式的类型推断要定义变量的类型，但不想用该表达式的值初始化变量，则：

```
deltype(f()) sum = x; // sum的类型就是函数f的返回类型。
```
...
比如数字、字符这些内置类型由C++直接定义，体现了大多数计算机硬件本身具备的能力。标准库定义了另外一组更高级性质的类型，它们尚未直接实现到计算机硬件中。

## 3.1 命名空间using声明

到目前为止，我们使用的库函数基本都属于命名空间`std`，比如`std::cin`表示使用命名空间std中名字cin。

- 简单写法，使用某个命名空间：例如 `using std::cin`表示使用命名空间`std`中的名字`cin`。统一写法为`using std::name`。
...
- 头文件中不应该包含`using`声明。如果头文件中有某个using声明，这样使用了该头文件的源码也会使用这个声明，会带来风险。
...
如果输入的是“  hello Word”，则实际读到的是“hello”。
...
**尽量使用c++版本的头文件**，即`cctype`，`cctype`头文件中定义了一组标准函数：
...
标准库vector表示对象集合，其中所有对象类型必须一致。因为其也能装其他对象，故也叫作**容器**。
...
**所有标准库容器（比如string、vector)都可以使用迭代器**，类似于指针类型，迭代器也提供了对对象的间接访问。
...
与vector类型，也是存放着类型相同的对象的容器，与vector不同的是，数组大小是确定不变的。
...
int *par[sz]; // 含有42个整形指针的数组
string bad[cnt]; // 错误 cnt不是常量表达式
```

`定义数组是必须指定数组的类型，不能用`auto`关键字有初始值来推断。`
...
需要注意字符串字面值结尾处还有一个空字符：

```
const char arr[5] = "hello"; // 错误 还需要多一位放置空字符
```
...
```
int *ptr[10];  // ptr是含有10个Int型指针的数组
int (*par)[10] = &arr; // par指向一个含有十个int型的数组
```

对于ptr，从右往左，先定义一个大小为10的数组，然后数组中存放的是指向int的指针。

对于par，从内往外，*par意味着par是个指针，接下来看下右边，par指向大小为10的数组的指针，最后观察左边，知道数组中元素是Int。
...
C++中使用数组时候编译器一般会把它转换为指针。在用到数组名字的地方，编译器会自动地将其替换为一个指向数组首元素的指针。

**标准库函数begin和end**

使用迭代器的前提是知道迭代对象的首元素地址和尾元素的下一位（即开始和结束的位置）
...
严格来说，C++中没有多维数组，通常说多维数组其实就是数组的数组。(有点套娃的含义)

当一个数组的元素仍是数组时，通常用两个维度去定义它：一个维度表示数组本身的大小，另外一个维度表示其元素(也是数组)的大小。

```
int a[3][4];  // 大小为3的数组，每个元素含有4个整数的数组
int b[10][20][30]; // 大小为10的数组，每个元素为大小20的数组，这些数组的元素是含有30和整数的数组。
```

对于二维数组，一般将第一个维度称为行，第二个维度称作列。
...
`int ia[3][4] = {{0}, {1}, {3}}`  , 仅仅初始化每行的首元素

**多维数组的for语句遍历**

使用范围for语句时，除了最内层的循环外，其他所有循环的控制变量都应该是**引用**类型。（要是仅仅读取就可以不需要了）
...
由于多维数组实际是多维数组的数组，所以由多维数组名转换来的指针实际上是指向第一个内层函数组的指针。

```
int ia[3][4];     // 大小为3的数组，每一个元素含有4个整数的数组
...
可以在if、while、for语句的控制结构内定义变量，该变量只在语句的内部可见，一旦语句结束，变量也就失效了。

## 5.3 条件语句

### 5.3.1 if语句

if语句：用来描述在嵌套的`if else`语句中，如果`if`比`else`多时如何处理的问题。C++使用的方法是`else`匹配最近没有配对的`if`。
...
- **函数形参列表**：可以为空，但不能省略。为了和C兼容，可以使用关键字`void`表示函数没有形参。
...
- **自动对象**：只存在于块执行期间的对象。当块的执行结束后，它的值就变成**未定义**的了。形参就是一种自动对象，函数开始时为形参申请存储空间，一旦函数终止，形参也被销毁。
- **局部静态对象**： `static`类型的局部变量，生命周期贯穿函数调用前后(一直都能用)
