...
插入迭代器又称插入器，本质上是一种迭代器的适配器。如前所诉，标准库算法为保证通用性，并不直接操作容器，而是通过迭代器来访问容器元素，因此算法不能直接向容器插入元素的能力。而插入器利用容器的成员函数即可实现插入的元素的能力。
...
int main()
{
  ...
  cout << endl;
  unique_copy(v.begin(), v.end(), out_iter);//疑问：为啥写到这还能正常输出
...
首先通过`cend`获得尾后的迭代器，循环过程中递减该迭代器，直到它与begin相等为止。注意，遍历所用的迭代器的初值为尾后位置，终值为begin之后的位置。
...
list是链表数据结构，元素不连续存储，其迭代器不支持算术运算。例如减法。
...
* `copy` 要求前两个参数至少**输入迭代器**，表示一个输入范围。它读取这个范围的元素，写入到第三个参数表示的输出序列中，故第三个参数至少是输出迭代器。
* `reverse` 需要反向处理序列，故要求两个参数至少是**双向迭代器**
* `unique`顺序扫描元素，覆盖重复的元素，因此前两个参数至少是**前向迭代器**。
...
* `replace_if` 在迭代器范围内，满足谓词条件pred的元素用新值替换。
