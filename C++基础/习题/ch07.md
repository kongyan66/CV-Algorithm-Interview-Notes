...
```
上述两个函数应该被定义为常量成员函数，因为不论返回姓名还是地址，在函数体只读取数据成员值，而不会做任何改变。
...
每个访问说明符指定了接下来的成员的访问级别，其有效范围直到出现下一个访问说明符或者达到类的结尾处为止。如果某个成员能够在整个程序内都被访问，那么它应该定义为`public`; 如果某个成员只能在类内部访问，那么它应该定义为`private`。
...
解：隐藏实现细节就是把数据成员以及不应被外部访问的函数设置成`private`。
...
解：所谓委托构造函数是指用它所属类的其他构造函数执行它自己的初始化过程。
...
class C {
public:
    C() : def(0) { } // 必须显示调用Nodefault的带参数构造函数初始化def
private:
    NoDefault def;
};
```
...
- (d) 不正确。对于编译器合成的默认构造函数来说，类类型的成员执行各自所属的默认构造函数，内置类型和符合类型的成员只对定义在全局作用域中的对象执行初始化。（属实没看懂）
...
解：`explict`优点是可以避免程序员不期望的隐式类型转换。

如果构造函数的接受的参数数量为1，就存在隐式转换问题，就可能需要指定`explicit`
