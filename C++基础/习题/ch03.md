...
解：如果设置为char，并不会对结果造成影响，因为auto自动推断字符串s的元素类型，结果同样是char。
...
解：不合法。因s未没有进行初始化，s内容为空，当然也就不存在首字符，下标0是非法的。
...
解：因为两个迭代器相互之间支持的运算只有 `-` ，而没有 `+` 。两个迭代器相减的结果是他们之间的距离。但是迭代器和迭代器差值（整数值）之间支持 `+`。
...
解：因为string类本身接受无参数的初始化形式，所以不论数组定义在内还是外部默认初始化为空串。对于内置类型Int来说，数组定位函数体之外默认初始化为0，定义在函数体之内的将不被初始化。
